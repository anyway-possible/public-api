  ...
  if (!serverPromise) {
    ...
    serverPromise = createX402Server({ apiKeyId: runtimeEnv.CDP_API_KEY_ID, apiKeySecret: runtimeEnv.CDP_API_KEY_SECRET, environment: "production", payToConfig: { type: "address", evm: PAY_TO }, builderCode: "anyway_possible", routes: {
      "POST /api/merchant-audit": {
        ...
        description: "Audit why an x402 API is not generating revenue. Analyze Coinbase Bazaar ranking, exact buyer searches, listing defects, competitor pricing, payment reliability, 30-day payer signals, and Base USDC activity. Repeated comparable audits return privacy-safe score history and decline alerts. No account or API key.",
...
async function paidHandler(request: NextRequest) {
  ...
  try {
    const result = await auditAndMonitorMerchant({ payTo: input.payTo, queries: input.queries as string[], excludePayers: input.excludePayers as string[] | undefined });
