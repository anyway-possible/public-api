  ...
  if (!serverPromise) {
    ...
    serverPromise = createX402Server({ apiKeyId: runtimeEnv.CDP_API_KEY_ID, apiKeySecret: runtimeEnv.CDP_API_KEY_SECRET, environment: "production", payToConfig: { type: "address", evm: PAY_TO }, builderCode: "anyway_possible", routes: {
      "POST /api/merchant-snapshot": {
        ...
        tags: ["x402 merchant analytics", "x402 API not selling", "increase x402 revenue", "x402 seller intelligence", "Coinbase Bazaar audit"],
...
async function paidHandler(request: NextRequest) {
  ...
  try {
    ...
    try { const identity = await identifyAgent(request); await getDb().insert(events).values({ eventId: crypto.randomUUID(), kind: identity.isSelfTest ? "test_call" : "paid_call", endpoint: "/api/merchant-snapshot", agentId: identity.agentId, clientType: identity.clientType, amountUsd: 0.05, costUsd: 0, latencyMs: Date.now() - started, statusCode: 200, network: "eip155:8453", occurredAt: result.observedAt }).run(); } catch {}
...
export async function POST(request: NextRequest) {
  try { const response = await withX402FromHTTPServer(paidHandler, await getServer())(request); if (response.status === 402) await recordPaymentChallenge(request, "/api/merchant-snapshot"); return response; }
