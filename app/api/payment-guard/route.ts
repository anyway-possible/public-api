  ...
  if (!serverPromise) {
    ...
    serverPromise = createX402Server({ apiKeyId: e.CDP_API_KEY_ID, apiKeySecret: e.CDP_API_KEY_SECRET, environment: "production", payToConfig: { type: "address", evm: PAY_TO }, builderCode: "anyway_possible", routes: {
      "POST /api/payment-guard": {
        ...
        tags: ["agent payment safety", "x402 payment safety", "verify before paying", "Base USDC preflight", "safe autonomous payment"],
...
export async function POST(request: NextRequest) { try { const response = await withX402FromHTTPServer(paidHandler, await getServer())(request); return await protectPaymentChallenge(request, "/api/payment-guard", response); } catch { await recordServiceError(request, "/api/payment-guard", 503); return NextResponse.json({ error: "Payment service is temporarily unavailable." }, { status: 503 }); } }
